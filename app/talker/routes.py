...
def on_socket_connect():
    """
    ✅ Realtime badge/dropdown bez pollingu:
    každý prihlásený user dostane svoju osobnú miestnosť: user:<id>
    """
    if not current_user.is_authenticated:
        return False  # odmietni connect

    try:
        join_room(f"user:{current_user.id}")
    except Exception:
        # nechceme zhodiť connect kvôli join_room
        pass
...
@talker.post("/update-unread-count/<int:room_id>")
def update_unread_count(room_id):
    # XHR: žiadne presmerovanie, len JSON
    if not current_user.is_authenticated:
        return jsonify({"error": "unauthorized"}), 401

    # v tejto route ignorujeme room_id, lebo menu potrebuje "global"
